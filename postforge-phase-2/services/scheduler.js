...
class SchedulerService {
  constructor(redisConfig = {}) {
    ...
    // Queue for scheduled generation jobs
    this.scheduleQueue = new Queue('schedule:generation', {
      redis: {
        host: redisConfig.host || 'localhost',
        port: redisConfig.port || 6379
      }
    ...
    // Queue for posting jobs
    this.postingQueue = new Queue('post:delivery', {
      redis: {
        host: redisConfig.host || 'localhost',
        port: redisConfig.port || 6379
      }
  ...
  setupProcessors() {
    ...
    this.scheduleQueue.process(async (job) => {
